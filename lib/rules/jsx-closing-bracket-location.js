  ...
  return {
    JSXOpeningElement: function(node) {
      ...
      context.report({
        ...
        data: data,
        fix: function (fixer) {
          var replaceRange = [node.range[1], node.range[1]];
          var replaceText;
          if (node.selfClosing) {
            replaceRange[0] -= 2;
            replaceText = '/>';
          ...
          if (expectedNextLine) {
            replaceText = '\n' + ' '.repeat(correctColumn) + replaceText;
          } else {
            var diff = tokens.closing.column - correctColumn;
            if (diff < 0) {
              replaceText = ' '.repeat(-diff) + replaceText;
            } else {
              replaceRange[0] -= diff;
            }
          }
